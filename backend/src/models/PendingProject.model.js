...
PendingProject.approveProject = async (pendingId) => {
    ...
    try {
        ...
        const values = [
            ...
            pendingProject.climate_relevance_score,
            pendingProject.climate_relevance_category,
            pendingProject.climate_relevance_justification,
            pendingProject.hotspot_vulnerability_type,
            pendingProject.wash_component_description,
            pendingProject.supporting_document,
        ];
        ...
        if (
            ...
            pendingProject.wash_component.presence
        ) {
            const { presence, wash_percentage, description } =
                pendingProject.wash_component;
            const washQuery = `
        ...
        await client.query("DELETE FROM PendingProject WHERE pending_id = $1", [
            pendingId,
        ]);
        await client.query("COMMIT");
